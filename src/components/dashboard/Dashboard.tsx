  ...
  const filteredPrograms = useMemo(() => {
    ...
    return programs.filter((program: any) => {
      ...
      if (user?.roleCode === 4) { // גורם דורש
        ...
        if (user.divisionId && program.divisionId !== user.divisionId) {
          return false;
        }
        if (user.departmentId && program.departmentId !== user.departmentId) {
          return false;
        }
      } else if (user?.roleCode === 3) { // קניין
        // Show only tasks assigned to this officer with specific statuses
        if (program.assignedOfficerId !== user.id) {
          return false;
        }
      } else if (user?.roleCode === 2) { // ראש צוות
      ...
      if (filters.quarter.length > 0) {
        const programQuarter = program.requiredQuarter ? 
          `Q${Math.ceil((new Date(program.requiredQuarter).getMonth() + 1) / 3)}/${new Date(program.requiredQuarter).getFullYear().toString().slice(-2)}` : 
          '';
    ...
    <div className="space-y-6">
      ...
      {filteredPrograms.length === 0 && (
        <EmptyState
          ...
          actionLabel={programs.length === 0 && [1, 4].includes(user?.roleCode || 0) ? 'צור משימה חדשה' : undefined}
          onAction={programs.length === 0 && [1, 4].includes(user?.roleCode || 0) ? () => navigate('/new-task') : undefined}
        />
      )}
    </div>
  );
